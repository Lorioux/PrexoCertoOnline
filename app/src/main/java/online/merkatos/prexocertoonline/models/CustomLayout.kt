...
import online.merkatos.prexocertoonline.R.styleable
        ...
        for (i in 0 until count) {
            ...
            if (child.visibility != View.GONE) {
                ...
                if (lp.position == LayoutParams.POSITION_LEFT) {
                    mLeftWidth += Math.max(maxWidth, child.measuredWidth + lp.leftMargin + lp.rightMargin)
                } else if (lp.position == LayoutParams.POSITION_RIGHT) {
                    mRightWidth += Math.max(maxWidth, child.measuredWidth + lp.leftMargin + lp.rightMargin)
                } else {
...
// Report our final dimensions.
        setMeasuredDimension(View.resolveSizeAndState(maxWidth, widthMeasureSpec, childState), View.resolveSizeAndState(maxHeight, heightMeasureSpec,
...
// Use the child's gravity and size to determine its final
    ...
    class LayoutParams : MarginLayoutParams {
        ...
        constructor(c: Context, attrs: AttributeSet) : super(c, attrs) {// Pull the layout param values from the layout XML during
            // inflation.  This is not needed if you don't care about
            // changing the layout behavior in XML.
            val a: TypedArray = c.obtainStyledAttributes(attrs, styleable.CustomLayoutLP)
            gravity = a.getInt(styleable.CustomLayoutLP_android_layout_gravity, gravity)
            position = a.getInt(styleable.CustomLayoutLP_layout_position, position)
            a.recycle()
        }
